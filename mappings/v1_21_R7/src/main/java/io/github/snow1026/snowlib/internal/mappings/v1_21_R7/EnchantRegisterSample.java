package io.github.snow1026.snowlib.internal.mappings.v1_21_R7;
...
public final class EnchantRegisterSample {
    ...
    private static final MappedRegistry<@NotNull Enchantment> ENCHANTS;
    private static final MappedRegistry<@NotNull Item> ITEMS;

    static {
        SERVER = ((CraftServer) Bukkit.getServer()).getServer();
        ENCHANTS = (MappedRegistry<@NotNull Enchantment>) SERVER.registryAccess().lookup(Registries.ENCHANTMENT).orElseThrow();
        ITEMS = (MappedRegistry<@NotNull Item>) SERVER.registryAccess().lookup(Registries.ITEM).orElseThrow();
    }
    ...
    public void register(SnowEnchantment snowEnchantment) {
        ...
        Identifier location = Identifier.fromNamespaceAndPath(snowKey.root(), snowKey.path());
        ResourceKey<@NotNull Enchantment> key = ResourceKey.create(Registries.ENCHANTMENT, location);

        HolderSet<@NotNull Item> supportedItems = HolderSet.direct(comp.supportedItems().materials().stream().map(this::getNmsItem).map(ITEMS::wrapAsHolder).collect(Collectors.toList()));
        Optional<HolderSet<@NotNull Item>> primaryItems = comp.primaryItems().map(set -> HolderSet.direct(set.materials().stream().map(this::getNmsItem).map(ITEMS::wrapAsHolder).collect(Collectors.toList())));
    ...
    private EquipmentSlotGroup getNmsSlotGroup(EquipmentSlot slot) {
        return switch (slot) {
